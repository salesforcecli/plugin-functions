...
class Javascript extends Template {
  ...
  writeLanguageFiles(tplConfig: TemplateConfig): void {
    ...
    this.writeFileFromTemplate({
      ...
      tplArgs: { fnNameHyphened, sfSdkVersion: SFDX_SDK_VERSION },
    });
...
export default class GenerateFunction extends Command {
  ...
  async isFirstFunction() {
    try {
      ...
      if (
        ['No functions directory found.', 'The functions directory does contain any functions.'].includes(error.message)
      ) {
        return true;
      }
  ...
  async run() {
    ...
    template.write({ fnDir, fnName, fnNameCased, isFunctionBundle });
