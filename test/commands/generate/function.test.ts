...
describe('sf generate:function', () => {
  ...
  function testTemplate(template: string, sfdxProjectPath: string | null) {
    return test
      ...
      .command(['generate:function', '--name=myfn', `--language=${template}`]);
  }

  // Javascript
  const javascriptBasicTemplateFiles = 7;
  testTemplate('javascript', 'sfdx-project.json').it('generates a javascript function', (ctx) => {
  ...
  testTemplate('typescript', 'sfdx-project.json').it('generates a typescript function', (ctx) => {
    expect(ctx.stderr).to.equal('');
  ...
  testTemplate('javascript', 'sfdx-project.json').stub(pathUtils, 'resolveFunctionsPaths', () => {
  ...
  test
    ...
    .command(['generate:function', '--name=myfn', '--language=javascript'])
...
describe('sf generate:function --language java', () => {
  ...
  function testJavaTemplate(sfdxProjectPath: string | null) {
    return test
      ...
      .command(['generate:function', '--name=myfn', '--language=java']);
  }

  const javaBasicFiles = 7;
  const javaTemplateFiles = 4;
  testJavaTemplate('sfdx-project.json').it('generates a java function', (ctx) => {
    expect(ctx.stderr).to.equal('');
