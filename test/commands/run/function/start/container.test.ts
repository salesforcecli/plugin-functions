...
describe('function:start:container', () => {
  ...
  test.command(['run:function:start:container']).it('Should call the library methods', async () => {
  ...
  context('with arguments', () => {
    test
      .command(['run:function:start:container', '-v', '--no-pull', '--clear-cache'])
    ...
    test
      .command(['run:function:start:container', '--debug-port=5001', '--port=5000'])
